      ...
      <v-container fluid class="pa-4 h-100 d-flex flex-column">
        ...
        <v-row class="mb-2 align-center">
            ...
            <v-col class="text-right text-caption text-grey">
                Showing {{ productStore.products.length }} items
            </v-col>
        </v-row>
        
        <!-- Loading State (Initial) -->
        <v-row v-if="productStore.loading && productStore.products.length === 0" justify="center" align="center" style="min-height: 400px;">
        ...
        <div v-else>
           ...
           <!-- Empty State -->
           <v-row v-if="productStore.products.length === 0 && !productStore.loading" justify="center" class="mt-16 text-center">
             <v-col cols="12">
           ...
           <v-row class="py-4" justify="center">
              <v-col cols="12" class="text-center">
                 ...
                 <div v-else-if="!productStore.hasMore && productStore.products.length > 0" class="text-caption text-grey mt-2">
                   End of results
                 </div>
              </v-col>
           </v-row>
...
const onIntersect = (isIntersecting: boolean, _entries: any[], _observer: any) => {
    // Safety check BEFORE loading more
    if (isIntersecting && 
        productStore.hasMore && 
        !productStore.loading && 
        !productStore.safetyBlock) {
