<template>
  <v-layout class="fill-height bg-grey-lighten-4">
    ...
    <v-navigation-drawer v-model="drawer" width="300" class="pa-4 bg-surface" elevation="2" floating>
      ...
      <!-- 1. Category -->
      <v-select v-model="selectedCategory" :items="categories" label="Category" variant="outlined" density="compact"
        class="mb-4"></v-select>
      ...
      <v-range-slider v-model="priceRange" :min="0" :max="3000" :step="50" color="primary" density="compact"
        hide-details class="mb-6" @end="() => applyServerFilters(false)"></v-range-slider>
      ...
      <v-expansion-panels variant="accordion" multiple>
        ...
        <v-expansion-panel v-if="selectedCategory === 'cpu'" title="Cores" elevation="0">
          <v-expansion-panel-text>
            <v-checkbox v-for="core in coreOptions" :key="core" v-model="filterCores" :label="String(core)"
              :value="core" density="compact" hide-details color="primary"></v-checkbox>
          </v-expansion-panel-text>
        </v-expansion-panel>
        ...
        <v-expansion-panel v-if="selectedCategory === 'video-card'" title="VRAM" elevation="0">
          <v-expansion-panel-text>
            <v-checkbox v-for="vram in vramOptions" :key="vram" v-model="filterVram" :label="vram" :value="vram"
        ...
        <v-expansion-panel v-if="selectedCategory === 'memory'" title="Size" elevation="0">
          <v-expansion-panel-text>
            <v-checkbox v-for="size in ramSizeOptions" :key="size" v-model="filterRamSize" :label="size" :value="size"
              density="compact" hide-details color="primary"></v-checkbox>
          </v-expansion-panel-text>
        </v-expansion-panel>
    ...
    <v-main>
      <v-container fluid class="pa-4 h-100 d-flex flex-column">
        ...
        <v-row class="mb-2 align-center">
          <v-col cols="auto">
            <v-btn v-if="!drawer" variant="text" color="primary" prepend-icon="mdi-filter" @click="drawer = true">
              Show Filters
            </v-btn>
          </v-col>
        ...
        <div v-else>
          <!-- Product Grid -->
          <v-row>
            <v-col v-for="product in productStore.products" :key="product.name" cols="12" sm="6" md="4" lg="3" xl="3">
              <v-card class="mx-auto rounded-xl product-card h-100 d-flex flex-column shadow-hover" elevation="3">
                <!-- Image / Icon Header -->
                <div class="gradient-bg d-flex align-center justify-center position-relative" style="height: 180px;">
                  ...
                  <v-chip v-if="!product.price" color="error" size="x-small"
                ...
                <v-card-text class="flex-grow-1">
                  ...
                  <div class="d-flex flex-wrap gap-1" style="max-height: 60px; overflow: hidden;">
                    <v-chip v-for="(val, key) in getImportantSpecs(product)" :key="key" size="x-small" variant="tonal"
                      class="mr-1 mb-1">
                      {{ val }}
                    </v-chip>
                  </div>
                ...
                <v-card-actions class="pa-3">
                  <v-btn :to="{ name: 'product-details', params: { id: product.id } }" text="Details" variant="tonal"
                    size="small" color="primary" class="flex-grow-1">
                    Details
                  </v-btn>
                  <v-btn icon="mdi-cart-plus" variant="tonal" color="secondary" size="small" class="ml-2"
                    @click="addToCart(product)"></v-btn>
                </v-card-actions>
              </v-card>
            </v-col>
          </v-row>
          ...
          <v-row class="py-4" justify="center">
            <v-col cols="12" class="text-center">
              <div v-intersect="onIntersect" style="height: 20px; width: 100%;"></div>

              <v-progress-circular v-if="productStore.loading" indeterminate color="primary"></v-progress-circular>

              <div v-else-if="!productStore.pagination.hasMore" class="text-caption text-grey mt-2">
...
const applyServerFilters = (isLoadMore = false) => {
...
const onIntersect = (isIntersecting: boolean) => {
  if (isIntersecting && productStore.pagination.hasMore && !productStore.loading) {
